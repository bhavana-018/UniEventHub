...
$contacts = $pdo->query("SELECT id, full_name, role, department FROM users WHERE role IN ('admin','student') AND is_active=1 ORDER BY role, full_name")->fetchAll();
    ...
    <div class="inbox-layout">
      ...
      <div class="chat-panel">
        ...
        <div class="chat-messages" id="chat-messages">
          <?php $prevDate=''; foreach ($chatMessages as $msg):
          ...
          <div class="msg-wrap <?= $isSent?'sent':'received' ?>">
            ...
            <div class="msg-body">
              ...
              <div class="msg-time"><?= date('g:i A',strtotime($msg['sent_at'])) ?><?php if ($isSent): ?><span class="msg-read-tick"><?= $msg['is_read']?' ✓✓':' ✓' ?></span><?php endif; ?></div>
            </div>
          </div>
          <?php endforeach; ?>
        ...
        <div class="chat-input-area">
          <div class="chat-input-wrap">
            <textarea class="chat-input" id="msg-input" rows="1" placeholder="Type a message… (Enter to send, Shift+Enter for new line)"></textarea>
          </div>
        ...
        <div class="chat-empty">
          ...
          <p>Choose from your existing conversations, or start a new message with a student or admin.</p>
        </div>
        <?php endif; ?>
      </div>
    ...
    <div class="modal-body">
      ...
      <div class="user-picker-list" id="contact-list">
        ...
        <div class="user-picker-item"
             data-id="<?= $c['id'] ?>" data-name="<?= htmlspecialchars(strtolower($c['full_name'])) ?>"
             onclick="selectContact(<?= $c['id'] ?>, '<?= htmlspecialchars($c['full_name'], ENT_QUOTES) ?>')">
          <div class="conv-avatar <?= $roleColors[$c['role']] ?? 'student' ?>" style="width:36px;height:36px;font-size:.8rem"><?= strtoupper(substr($c['full_name'],0,1)) ?></div>
          <div>
...
document.getElementById('contact-search').addEventListener('input', function() {
  const q = this.value.toLowerCase();
  document.querySelectorAll('.user-picker-item').forEach(i => { i.style.display = i.dataset.name.includes(q)?'':'none'; });
});

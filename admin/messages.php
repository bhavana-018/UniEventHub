...
$conversations = $pdo->prepare("
    SELECT
        ...
        m.message AS last_message, m.sent_at AS last_time, m.sender_id,
...
$contacts = $pdo->prepare("SELECT id, full_name, role, department FROM users WHERE is_active=1 AND id != ? ORDER BY role, full_name");
$contacts->execute([$uid]); $contacts = $contacts->fetchAll();
      ...
      <div class="conversations-panel">
        ...
        <div class="conversations-list">
          ...
          <a href="messages.php?with=<?= $c['id'] ?>"
            ...
            <div class="conv-avatar <?= $roleColors[$c['role']] ?? 'student' ?>"><?= strtoupper(substr($c['full_name'],0,1)) ?></div>
            <div class="conv-info">
            ...
            <div class="conv-meta">
              ...
              <?php if ($c['unread_count'] > 0): ?><div class="conv-badge"><?= $c['unread_count'] ?></div><?php endif; ?>
            </div>
          </a>
          <?php endforeach; ?>
          <?php endif; ?>
      ...
      <div class="chat-panel">
        ...
        <div class="chat-messages" id="chat-messages">
          ...
          <div class="msg-wrap <?= $isSent?'sent':'received' ?>">
            <?php if (!$isSent): ?>
            <div class="msg-avatar-sm <?= $roleColors[$msg['sender_role']] ?? 'student' ?>"><?= strtoupper(substr($msg['sender_name'],0,1)) ?></div>
            <?php endif; ?>
            <div class="msg-body">
              <div class="msg-bubble"><?= nl2br(htmlspecialchars($msg['message'])) ?></div>
              <div class="msg-time">
                <?= date('g:i A', strtotime($msg['sent_at'])) ?>
                <?php if ($isSent): ?><span class="msg-read-tick"><?= $msg['is_read']?' ✓✓':' ✓' ?></span><?php endif; ?>
              </div>
            </div>
          </div>
          <?php endforeach; ?>
        ...
        <div class="chat-empty">
          ...
          <p>Choose from your existing conversations, or start a new message with any user.</p>
        </div>
        <?php endif; ?>
      </div>
  ...
  <div class="modal">
    ...
    <div class="modal-body">
      ...
      <div style="display:flex;gap:6px;margin-bottom:10px;flex-wrap:wrap">
        <button class="btn btn-primary btn-sm role-filter" data-role="all" onclick="filterContacts()">All</button>
        <button class="btn btn-ghost btn-sm role-filter" data-role="student" onclick="filterContacts()">Students</button>
        <button class="btn btn-ghost btn-sm role-filter" data-role="organizer" onclick="filterContacts()">Organizers</button>
      </div>
      <div class="user-picker-list" id="contact-list">
        ...
        <div class="user-picker-item"
          ...
          <div class="conv-avatar <?= $roleColors[$c['role']] ?? 'student' ?>" style="width:36px;height:36px;font-size:.8rem">
        ...
        <div style="text-align:center;padding:24px;color:var(--text-5)">No users available</div>
        <?php endif; ?>
      </div>
...
if (msgInput) {
  ...
  <?php if ($activeId): ?> setTimeout(() => msgInput.focus(), 80); <?php endif; ?>
}
...
async function sendMessage() {
  ...
  try {
    ...
    fd.append('receiver_id', ACTIVE_ID); fd.append('message', sent);
...
function appendBubble(side, text, time, tick) {
  const chatEl = document.getElementById('chat-messages'); if (!chatEl) return;
  const wrap = document.createElement('div'); wrap.className = 'msg-wrap ' + side;
  ...
  if (side === 'received') {
    ...
    wrap.innerHTML = `<div class="msg-avatar-sm ${role}">${init}</div><div class="msg-body"><div class="msg-bubble">${safe}</div><div class="msg-time">${time}</div></div>`;
  } else {
    wrap.innerHTML = `<div class="msg-body"><div class="msg-bubble">${safe}</div><div class="msg-time">${time}${tickHtml}</div></div>`;
  }
  chatEl.appendChild(wrap); scrollToBottom(true);
}
...
setInterval(async() => {
  try {
    ...
    if (data.messages && data.messages.length > 0) {
      data.messages.forEach(m => { appendBubble('received', m.raw||m.message, m.time, ''); lastId = Math.max(lastId, m.id||0); });
    }
  } catch {}
}, 4000);
<?php endif; ?>
...
document.getElementById('conv-search').addEventListener('input', function() {
  ...
  document.querySelectorAll('.conversation-item').forEach(i => { i.style.display = i.dataset.name.includes(q)?'':'none'; });
});
...
document.querySelectorAll('.role-filter').forEach(btn => {
  btn.addEventListener('click', function() {
    activeRoleFilter = this.dataset.role;
    document.querySelectorAll('.role-filter').forEach(b => { b.className = 'btn btn-ghost btn-sm role-filter'; });
    this.className = 'btn btn-primary btn-sm role-filter';
    filterContacts();
  });
});
function filterContacts() {
  ...
  document.querySelectorAll('.user-picker-item').forEach(item => {
    const nameOk = item.dataset.name.includes(q);
    const roleOk = activeRoleFilter === 'all' || item.dataset.role === activeRoleFilter;
    item.style.display = (nameOk && roleOk) ? '' : 'none';
  });
}
...
function selectContact(id, name) {
  ...
  const el = document.querySelector(`.user-picker-item[data-id="${id}"]`); if (el) el.classList.add('selected');
  const sel = document.getElementById('selected-contact'); sel.style.display='block'; sel.textContent='✓ Selected: '+name;
}
...
async function sendNewMessage() {
  ...
  if (!text) { alertEl.className='alert alert-danger'; alertEl.innerHTML='✕ Please enter a message.'; alertEl.style.display='flex'; return; }
  btn.disabled = true; btn.textContent = 'Sending…';
  try {
    const fd = new FormData(); fd.append('receiver_id', selectedContactId); fd.append('message', text);
    const res  = await fetch('../php/send_message.php', { method:'POST', body:fd });
    const data = await res.json();
    if (data.success) { window.location.href = 'messages.php?with=' + selectedContactId; }
    else { alertEl.className='alert alert-danger'; alertEl.innerHTML='✕ '+(data.message||'Failed.'); alertEl.style.display='flex'; btn.disabled=false; btn.textContent='Send Message'; }
  } catch { showToast('Network error.','error'); btn.disabled=false; btn.textContent='Send Message'; }
}

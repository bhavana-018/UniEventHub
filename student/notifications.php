    ...
    <div class="page-content">
      ...
      <div class="card">
        ...
        <div class="empty-state">
          ...
          <p>Event updates and alerts will appear here</p>
        </div>
        <?php else: ?>
        <?php foreach ($notifs as $n): ?>
        <div style="display:flex;align-items:flex-start;gap:14px;padding:16px 20px;border-bottom:1px solid var(--border);transition:var(--t)" onmouseenter="this.style.background='rgba(99,102,241,.06)'" onmouseleave="this.style.background=''">
          <div style="width:36px;height:36px;border-radius:50%;background:var(--bg-4);display:flex;align-items:center;justify-content:center;flex-shrink:0;margin-top:2px">
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="var(--indigo-light)" stroke-width="2"><path d="M18 8A6 6 0 0 0 6 8c0 7-3 9-3 9h18s-3-2-3-9"/><path d="M13.73 21a2 2 0 0 1-3.46 0"/></svg>
          </div>
          <div style="flex:1;min-width:0">
            <div style="font-size:.84rem;color:var(--text-1);line-height:1.5"><?= htmlspecialchars($n['message']) ?></div>
            <div style="font-size:.72rem;color:var(--text-5);margin-top:4px"><?= date('M j, Y · g:i A', strtotime($n['created_at'])) ?></div>
          </div>
          <span class="badge badge-nodot badge-success">Read</span>
        </div>
        <?php endforeach; ?>
        <?php endif; ?>
      </div>
